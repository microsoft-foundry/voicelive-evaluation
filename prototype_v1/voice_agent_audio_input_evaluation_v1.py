...
session_timestamp_global = None  # Single session timestamp for all outputs
...
class ConversationMetrics:
    ...
    def finalize_turn_and_get_evaluation_data(self):
        ...
        evaluation_data = {
            ...
            "metrics": metrics,
            "turn_info": {
                "logical_turn_number": self.logical_turn_number,
                "conversation_topic": self.conversation_topic,
                "inputs_in_turn": len(current_user_inputs),
                "responses_in_turn": len(current_assistant_responses)
            }
        }
...
def main(test_files_path: str = None, output_dir: str = None, evaluation_dir: str = None, session_timestamp: str = None) -> None: 
    ...
    global evaluation_enabled, evaluation_output_file, session_timestamp_global
    session_timestamp_global = session_timestamp  # Store globally for other functions
    if evaluation_dir:
        ...
        # Set the evaluation output file using session timestamp
        evaluation_output_file = os.path.join(evaluation_dir, f"evaluation_{session_timestamp}.jsonl")
    ...
    # instructions = "You are a helpful agent trying to answer questions of users."
    turn_detection = {
    ...
    if output_dir is None:
        output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", session_timestamp_global)
    else:
        output_dir = os.path.join(output_dir, session_timestamp_global)
...
def read_test_files(test_files_path: str = None) -> List[str]:
    ...
    if test_files_path is None:
        ...
        test_files_path = os.path.join(script_dir, "test_files.txt")
...
def update_output_file_for_new_turn():
    ...
    if response_output_dir:
        ...
        current_output_file = os.path.join(response_output_dir, f"turn_{current_turn_number:02d}_response.wav")
...
def write_operational_metrics_summary():
    ...
    if evaluation_enabled and evaluation_output_file:
        ...
        # Write summary to a separate file using session timestamp
        evaluation_dir = os.path.dirname(evaluation_output_file)
        summary_file = os.path.join(evaluation_dir, f"operational_summary_{session_timestamp_global}.json")
        ...
        # Even without evaluation mode, print the summary
                ...
                if (inactivity_time > inactivity_timeout and not has_recent_audio \
                    ...
                    if evaluation_enabled and evaluation_output_file and current_metrics.audio_send_end_time:
                        try:
                            ...
                            if evaluation_data:
                                print(f"DEBUG - Timeout: Turn {evaluation_data['turn_info']['logical_turn_number']} evaluation data: {json.dumps(evaluation_data)[:200]}...")
                                with open(evaluation_output_file, 'a', encoding='utf-8') as f:
                                    f.write(json.dumps(evaluation_data) + '\n')
                                print(f"Timeout: Turn {evaluation_data['turn_info']['logical_turn_number']} evaluation data written")
                            else:
                    ...
                    if evaluation_enabled and evaluation_output_file:
                        try:
                            if current_metrics.audio_send_end_time and not current_metrics.current_turn_complete:
                                ...
                                if evaluation_data:
                                    print(f"DEBUG - Turn {evaluation_data['turn_info']['logical_turn_number']} evaluation data: {json.dumps(evaluation_data)[:200]}...")
                                    with open(evaluation_output_file, 'a', encoding='utf-8') as f:
                                        f.write(json.dumps(evaluation_data) + '\n')
                                    print(f"Turn {evaluation_data['turn_info']['logical_turn_number']} evaluation data written to {evaluation_output_file}")
                                else:
                        ...
                        if not current_metrics.current_turn_complete and current_metrics.audio_send_end_time and evaluation_output_file:
                            try:
                                ...
                                if evaluation_data:
                                    with open(evaluation_output_file, 'a', encoding='utf-8') as f:
                                        f.write(json.dumps(evaluation_data) + '\n')
                                    print(f"DEBUG - Wrote pending turn {evaluation_data['turn_info']['logical_turn_number']} on speech start")
                                else:
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument('--test-files', '-f', dest='test_files_path', default='./test_files.txt',
        ...
        try:
            if evaluation_enabled:
                try:
                    import voice_agent_evaluation_v1
                except ImportError as e:
                    print(f"Error importing evaluation module: {e}")
                print("Starting evaluation...")
                ## Change the eval_input_path to point to your evaluation data file
                eval_input_path = evaluation_output_file
                eval_name = os.path.basename(evaluation_output_file)
                eval_description = f"Evaluation of Voice Live API agent: {os.path.basename(evaluation_output_file)}"
                voice_agent_evaluation_v1.main(eval_input_path, eval_name, eval_description, f'./{args.output_dir}/{timestamp}_evaluation_results')
